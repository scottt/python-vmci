/* python-2.7's socket module hacked to add VMWare VMCI support, see:
 * http://www.vmware.com/pdf/ws7_esx4_vmci_sockets.pdf
 * */
...
- Only AF_INET, AF_INET6 and AF_UNIX address families are supported in a
  ...
  under Linux. AF_VMCI (VMCISock_GetAFValue()) support was hacked in later.
...
- names starting with PySocket are exported through vmcimodule.h
...
static PyObject *
vmci_get_local_cid(PyObject *self, PyObject *unused)
{
    unsigned int cid;
...
PyDoc_STRVAR(vmci_get_local_cid_doc, "get_local_cid(): return the context ID of the local virtual machine");
...
static PyMethodDef socket_methods[] = {
    ...
    {"get_local_cid",           vmci_get_local_cid, METH_NOARGS, vmci_get_local_cid_doc},
    {NULL,                      NULL}            /* Sentinel */
};
...
init_vmci(void)
{
    PyObject *m, *has_ipv6, *socketmodule;

    if (!os_init())
        return;

    if ((af_vmci = VMCISock_GetAFValue()) < 0)
        return; /* FIXME: fails for non-root users by default */
    ...
    /* keeping 'af_vmci' in lower case as its value is from VMCISock_GetAFValue() and
     * thus not really a constant */
    PyModule_AddIntConstant(m, "af_vmci", af_vmci);
    PyModule_AddIntMacro(m, SO_VMCI_BUFFER_SIZE);
    PyModule_AddIntMacro(m, SO_VMCI_BUFFER_MIN_SIZE);
    PyModule_AddIntMacro(m, SO_VMCI_BUFFER_MAX_SIZE);
    PyModule_AddIntMacro(m, SO_VMCI_PEER_HOST_VM_ID);

    PyModule_AddIntMacro(m, VMADDR_CID_ANY);
    PyModule_AddIntMacro(m, VMADDR_PORT_ANY);

    socketmodule = PyImport_ImportModule("socket");
    if (socketmodule == NULL)
        return;

    socket_error = PyObject_GetAttrString(socketmodule, "error");
    if (socket_error == NULL)
        return;
    PySocketModuleAPI.error = socket_error;
    Py_INCREF(socket_error);
    PyModule_AddObject(m, "error", socket_error);
    socket_herror = PyObject_GetAttrString(socketmodule, "herror");
    if (socket_herror == NULL)
        return;
    Py_INCREF(socket_herror);
    PyModule_AddObject(m, "herror", socket_herror);
    socket_gaierror = PyObject_GetAttrString(socketmodule, "gaierror");
    if (socket_gaierror == NULL)
        return;
    Py_INCREF(socket_gaierror);
    PyModule_AddObject(m, "gaierror", socket_gaierror);
    socket_timeout = PyObject_GetAttrString(socketmodule, "timeout");
